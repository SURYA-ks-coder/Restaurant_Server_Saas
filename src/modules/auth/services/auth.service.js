...
const login = async ({ email, password, branchId }) => {
  try {
    ...
    if (
      branchId &&
      !user.branchIds.map(String).includes(String(branchId)) &&
      !isOwner
    ) {
      throw new AppError(
        "User does not have access to selected branch",
...
const resetPassword = async ({ token, password }) => {
  const user = await userRepository
    ...
    .select(
      "+passwordResetTokenHash +passwordResetExpiresAt +password +tokenVersion",
    );
  if (!user)
    throw new AppError(
